...
public class AddUser extends TestBase {
    ...
    @Test(priority = 3, dataProviderClass = DataUtils.class, dataProvider = "addUserData")
    @Description("Admin will add user password")
    @Feature("Add users")
    @Story("As a Admin I need to add user email address")
    public void add_Pass_And_Confirm_Password(String data)  {
        String[] formInfo= data.split(",");
